import { curry } from 'ramda'
import { FILES, SIDE, MOVEMENTS } from '~/constants'
import { isEmpty, isExist, isEven, parseInt10, split } from '~/utils'

/**
 * Get file as number
 * @param  {string} fileName
 * @return {number}
 */
function parseFileNum (fileName) {
  const idx = FILES.indexOf(fileName)

  if (idx > -1) {
    return idx + 1
  }

  return idx
}

/**
 * Get rank name as number
 * @alias
 */
const parseRankNum = parseInt10

/**
 * Get file and rank name
 * @param  {string} tileName
 * @return {Object}
 */
function getFileRankName (tileName) {
  const splitedTile = split(tileName)

  if (splitedTile.length > 2) {
    return {}
  }

  return splitedTile.reduce((acc, name) => {
    const keyName = /[1-9]/.test(name) ? 'rankName' : 'fileName'

    return {
      ...acc,
      [keyName]: name
    }
  }, {})
}

/**
 * Get side
 * @param  {string} key
 * @return {string}
 */
function getSideBy (key) {
  return SIDE[key]
}

/**
 * Get movements tiles
 * @param  {string} tileName
 * @param  {string} piece
 * @param  {string} turn
 * @return {Array}
 */
const getMovements = curry((tileName, piece, turn) => {
  const mvs = MOVEMENTS[piece]
  const { rankName, fileName } = getFileRankName(tileName)
  const x = parseFileNum(fileName)
  const y = parseRankNum(rankName)

  return mvs.map((mv) => {
    const [mvX, mvY] = mv
    const nextX = mvX + x
    const nextY = turn === 'white' ? mvY + y : y - mvY

    return [nextX, nextY]
  })
})

/**
 * Get pure movable tiles (no filter)
 * @param  {Array} movable
 * @return {Array}
 */
function getPureMovable (movable) {
  const filteredMovable = movable.reduce((acc, mvs) => {
    const [file, rank] = mvs
    const nextFile = FILES[file - 1]
    const nextTile = `${nextFile}${rank}`
    const inUndefined = isEmpty(nextFile)

    // TODO: find out why it happens
    const nonTileName = rank === 0 || nextTile.indexOf('-') > -1

    if (inUndefined || nonTileName) {
      return acc
    }

    return [...acc, nextTile]
  }, [])

  return filteredMovable
}

/**
 * Get next notations
 * @param  {string} selected
 * @param  {string} nextTileName
 * @param  {Array}  prevNotations
 * @return {Array}
 */
const getNextNotations = curry((selected, nextTileName, prevNotations) => {
  const { selectedTile } = getSelectedItems(selected)

  return prevNotations.reduce((acc, notation) => {
    if (notation.indexOf(selectedTile) > -1) {
      const [side, piece] = split(notation)

      return [...acc, `${side}${piece}${nextTileName}`]
    }

    return [...acc, notation]
  }, [])
})

function movableWithDirection (movableTiles) {
  const initialObj = {
    vertical: [],
    horizontal: [],
    diagonal: [],
    pending: '',
    lastKey: ''
  }

  return movableTiles.reduce((acc, tile, idx) => {
    const { pending, lastKey } = acc
    const [currentFile, currentRank] = split(tile)
    const currentFileNum = parseFileNum(currentFile)
    const currentRankNum = parseRankNum(currentRank)

    // first or last
    if (isEmpty(pending)) {
      if (isExist(lastKey) && movableTiles.length === idx + 1) {
        return {
          ...acc,
          [lastKey]: [...acc[lastKey], tile],
          pending: '',
          lastKey: ''
        }
      }

      return {
        ...acc,
        pending: tile
      }
    }

    const [pendingFile, pendingRank] = split(pending)
    const pendingFileNum = parseFileNum(pendingFile)
    const pendingRankNum = parseRankNum(pendingRank)
    const x = Math.abs(pendingFileNum - currentFileNum)
    const y = Math.abs(pendingRankNum - currentRankNum)

    if (x === 1 && y === 1) {
      return {
        ...acc,
        diagonal: [...acc.diagonal, pending, tile],
        lastKey: 'diagonal',
        pending: ''
      }
    }

    if (x === 0) {
      return {
        ...acc,
        vertical: [...acc.vertical, pending, tile],
        lastKey: 'vertical',
        pending: ''
      }
    }

    if (y === 0) {
      return {
        ...acc,
        horizontal: [...acc.horizontal, pending, tile],
        lastKey: 'horizontal',
        pending: ''
      }
    }

    return {
      ...acc,
      [lastKey]: [...acc[lastKey], pending],
      pending: tile,
      lastKey: ''
    }
  }, initialObj)
}

function getSelectedItems (selected) {
  const [selectedTile, selectedSide] = split.by('-', selected)

  return { selectedTile, selectedSide }
}

const isDarkBackground = curry((fileName, rankName) => {
  const evenTiles = ['b', 'd', 'f', 'h']
  const oddTiles = ['a', 'c', 'e', 'g']
  const darkTiles = isEven(rankName) ? evenTiles : oddTiles

  return darkTiles.includes(fileName)
})

export {
  parseFileNum,
  parseRankNum,
  getSideBy,
  getFileRankName,
  getSelectedItems,
  getNextNotations,
  getMovements,
  getPureMovable,
  movableWithDirection,
  isDarkBackground
}
