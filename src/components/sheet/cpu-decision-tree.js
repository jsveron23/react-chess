...
const C2 = 98;   // col 2 x — opponent's reply (NW + 18px gap)
const C3 = 196;  // col 3 x — CPU's counter (2 × (NW + 18px))
...
const TreeNode = ({ x, y, label, rankLabel, score, isHighlight }) => {
  ...
  return (
    <g>
      ...
      {rankLabel && (
        <text x={x + 4} y={y + 11} fontSize={8} fill="#888" fontFamily="monospace">
          {rankLabel}
        </text>
      )}
      <text
...
const CpuDecisionTree = ({ decisionTree, color }) => {
  ...
  const svgW = hasCounter ? SVG_W_3 : SVG_W_2;
