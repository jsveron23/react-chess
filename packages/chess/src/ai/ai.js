...
class AI {
   ...
   * @param {object} state - Move state
   * @return {number} MVV-LVA score
   */
  static #mvvLva(state) {
   ...
   * @param {Array} stateList - List of move states
   * @return {Array} Sorted move states
   */
   ...
   * @param {Array} snapshot - Board snapshot
   * @return {number} Pawn structure score
   */
   ...
   * @param {boolean} isMaximisingPlayer - Whether maximising player
   * @param {number} [qDepth=0] - Current quiescence depth
   * @return {number} Quiescence score
   */
  ...
  static minimax(currState, depth, alpha, beta, isMaximisingPlayer) {
    ...
    for (let i = 0, len = orderedList.length; i < len; i++) {
      ...
      if (isMaximisingPlayer) {
        ...
        localAlpha = Math.max(localAlpha, bestMove);
      } else {
        bestMove = Math.min(bestMove, score);
        localBeta = Math.min(localBeta, bestMove);
      }
   ...
   * @param {string} side - Side identifier ('w' or 'b')
   * @param {Array} snapshot - Board snapshot
   * @return {Array} Filtered piece codes
   */
